...
```{r tf_logfc_rank}
markers3$IsTF <- ifelse(markers3$AMEXID %in% TFs$value, TRUE, FALSE)
...
annotation.df <- read.csv("Raw.data/AnnotationFile_AmexG_v6_chr_unscaffolded_CherryGFP_Ines.tsv", header = TRUE, sep = "\t")
dea.tf <- merge(dea.tf, annotation.df, by = "AMEXID", all.x = TRUE)

dea.tf <- dea.tf %>% arrange(-abs(avg_log2FC))
```

```{r update_plot_revision}
# Make candidate plot more interpretable
# Focus on top right and bottom left corners of scatter plot (highlighted in manuscript)
# Top right: BL hubs, Bottom left: CSD hubs
candidate_df_bl <- df.zfilter.diffex %>%
  filter(IsTF == TRUE) %>%
  filter(p_val_adj < 0.05) %>%
  filter(CompareRanks_Zscore > 0 & avg_log2FC > 0) %>%
  mutate(score = CompareRanks_Zscore * avg_log2FC,
         grn = "BL") %>%
  arrange(-score) %>%
  head(n = 5)
  

candidate_df_csd <- df.zfilter.diffex %>%
  filter(IsTF == TRUE) %>%
  filter(p_val_adj < 0.05) %>%
  filter(CompareRanks_Zscore < 0 & avg_log2FC < 0) %>%
  mutate(score = CompareRanks_Zscore * avg_log2FC,
...
candidates_bl_plt <- ggplot(candidate_df_bl, aes(x = score, y = reorder(Symbol, score), fill = avg_log2FC)) +
  geom_col(stat = "identity", color = "black", width = 0.75) + 
  theme_bw() +
  scale_fill_gradient(low = "white", high = "forestgreen", limits = c(0, 2)) +
  labs(title = "Top 5 TF candidates in Blastema GRN",
       x = "score",
       y = "TF") +
  theme(plot.title = element_text(hjust = 0.5, face = "bold"))


candidates_csd_plt <- ggplot(candidate_df_csd, aes(x = score, y = reorder(Symbol, score), fill = avg_log2FC)) +
  geom_bar(stat = "identity", color = "black", width = 0.75) + 
  theme_bw() +
  scale_fill_gradient(low = "hotpink3", high = "white", limits = c(-2, 0)) +
  labs(title = "Top 5 TF candidates in CSD GRN",
       x = "score",
       y = "TF") +
  theme(plot.title = element_text(hjust = 0.5, face = "bold"))

pdf("Figures/Barplot_CandidateTFs_TENET_Blastema.pdf", height = 3, width = 4)
candidates_bl_plt
dev.off()

pdf("Figures/Barplot_CandidateTFs_TENET_CSD.pdf", height = 3, width = 5)
candidates_csd_plt
dev.off()
