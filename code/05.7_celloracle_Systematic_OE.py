###################################################################
#                                                                 #
#                CELLORACLE STEP 8: IN SILICO OE                  #
#                                                                 #
###################################################################
...
injury = "injury"
oevalue = "MeanExpression2" # Other options are: "MaxExpression", "MaxExpression2", "MeanExpression", "MeanExpression2"
...
n_propagation = 3
file_path = "/home/iriverog/NetVolumes/LAB_MT/RESULTADOS/Ines/Axolotl_Limb_Regeneration/Results/celloracle/BLct_CSDct_OE_" + oevalue + "_screen.celloracle.hdf5"
ps_file = "/home/iriverog/NetVolumes/LAB_MT/RESULTADOS/Ines/Axolotl_Limb_Regeneration/Results/celloracle/BLct_CSDct_TF_candidates_OE_" + oevalue + "_Simulation_Result.tsv"
...
def pipeline(gene_for_OE):
    ...
    if oevalue == "MaxExpression":
        newvalue = sc.get.obs_df(oracle.adata, keys=[gene_for_OE], layer="imputed_count").max() # get max value
    if oevalue == "MaxExpression2":
        newvalue = sc.get.obs_df(oracle.adata, keys=[gene_for_OE], layer="imputed_count").max() # get max value and multiply times 2
        newvalue = newvalue * 2
    if oevalue == "MeanExpression":
        newvalue = sc.get.obs_df(oracle.adata, keys=[gene_for_OE], layer="imputed_count").mean() # get mean value
    if oevalue == "MeanExpression2":
        newvalue = sc.get.obs_df(oracle.adata, keys=[gene_for_OE], layer="imputed_count").mean() # get mean value and multiply times 2
        newvalue = newvalue * 2
    ...
    print("Simulating overexpression")
    # 1. Simulate KO
    oracle.simulate_shift(perturb_condition={gene_for_OE: newvalue}, ignore_warning=True, n_propagation=n_propagation)
    ...
    for lineage_name, cell_idx in index_dictionary.items():
        ...
        PSgrid = "/home/iriverog/NetVolumes/LAB_MT/RESULTADOS/Ines/Axolotl_Limb_Regeneration/Results/celloracle/BLct_CSDct_OE_" + oevalue + "_" + lineage_name + "_" + gene_for_OE + "_PSgrid.pdf"
        PSgridvector = "/home/iriverog/NetVolumes/LAB_MT/RESULTADOS/Ines/Axolotl_Limb_Regeneration/Results/celloracle/BLct_CSDct_OE_" + oevalue + "_"  + lineage_name + "_" + gene_for_OE + "_PSvector.pdf"
        vectorfieldsplot = "/home/iriverog/NetVolumes/LAB_MT/RESULTADOS/Ines/Axolotl_Limb_Regeneration/Results/celloracle/BLct_CSDct_OE_" + gene_for_OE + "VectorsFieldPlot.pdf"
        vectorwithcluster = "/home/iriverog/NetVolumes/LAB_MT/RESULTADOS/Ines/Axolotl_Limb_Regeneration/Results/celloracle/BLct_CSDct_OE_" + oevalue + "_"  + gene_for_OE + "VectorsFieldWithCluster.pdf"
        ...
        ax[0].set_title(f"Simulated cell identity shift vector: {gene_for_OE} overexpression {oevalue}")
